...
(function(B) {
	var _ = "../script/upload/images/loading.jpg",
		C = "",
		A = "maxWidth" in document.documentElement.style,
		$ = !-[1, ] && !("prototype" in Image) && A;
	new Image().src = _;
	B.fn.autoIMG = function() {
		var B = this,
			$ = B.width();
		return B.find("img").each(function(H, E) {
			if (A) return E.style.maxWidth = $ + "px";
			var D = E.getAttribute("data-src") || E.src,
				B = E.nextSibling,
				G = E.parentNode,
				F = new Image();
	...
	(function() {
		var B = [],
			...
			_ = function() {
				var _ = 0;
				for (; _ < B.length; _++) B[_].end ? B.splice(_--, 1) : B[_]();
				!B.length && $()
			},
			$ = function() {
				clearInterval(A);
		...
		this.imgReady = function(G, L, $) {
			var C, P, D, J, N, I, F, O = 1024,
				M = document,
				K = M.body || M.getElementsByTagName("head")[0],
				E = new Image();
			E.src = G;
			if (!L) return E;
			if (E.complete) return L(E.width, E.height);
			...
			!C.end && C();
			for (var H = 0; H < B.length; H++) if (B[H].url === G) return;
			if (!C.end) {
				B.push(C);
				if (!A) A = setInterval(_, 150)
			}
		}
	})()
})(jQuery)